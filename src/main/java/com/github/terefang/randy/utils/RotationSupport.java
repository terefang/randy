 ...
 * {@link #randomRotation3D(long, double[])} or use {@link #randomRotation3D(long)} to make a completely new one. The
 * same follows for making a 4D rotation using a 3D one, and so on. You can apply a rotation to a vector with
 * {@link #rotate(double[], double[], double[])}, and there's also a method to multiply square matrices in
 * {@link #matrixMultiply(double[], double[], double[], int)}.
 * <br>
...
public final class RotationSupport {
    private static final double ROOT2 = 1.4142135623730950488f;
    ...
    public static double[] rotateStep(long seed, double[] small, int targetSize) {
        ...
        for (int i = 0; i < targetSize; i++) {
            gauss[i] = t = (double) Scramblers.scrambleGaussian(sd += 0x9E3779B97F4A7C15L);
            sum += t * t;
        }
        final double inv = 1f / (double) Math.sqrt(sum);
        sum = 0f;
        t = 1f;
        for (int i = 0; i < targetSize; i++) {
            t -= gauss[i] *= inv;
        ...
        sum = ROOT2 / (double) Math.sqrt(sum); // reused as what the subgroup paper calls c
        t = 1f;
        for (int i = 0; i < targetSize; i++) {
     ...
     * {@link #rotate(double[], double[], double[])}. Scrambles the given seed with {@link Scramblers#scrambleFloat(long)},
     ...
     */
    public static double[] randomRotation2D(long seed) {
        final double theta = Scramblers.scrambleFloat(seed) * 6.2831855f;
     ...
     */
    public static double[] fillRandomRotation2D(long seed, double[] out) {
        final double theta = Scramblers.scrambleFloat(seed) * 6.2831855f;
        final double s = Math.sin(theta);
        final double c = Math.cos(theta);
        out[0] = out[3] = c;
     ...
     */
    public static double[] randomRotation3D(long seed) {
        return rotateStep(seed, randomRotation2D(seed - 3), 3);
    }

    /**
     * Creates a new 1D double array that can be used as a 3D rotation matrix by
     * {@link #rotate(double[], double[], double[])}. Uses the given long seed to get Gaussian floats using
     ...
     */
    public static double[] randomRotation3D(long seed, double[] rotation2D) {
     ...
     * @return a newly-allocated 16-element double array, meant as effectively a 4D rotation matrix
     */
    public static double[] randomRotation4D(long seed) {
     ...
     * @return a newly-allocated 16-element double array, meant as effectively a 4D rotation matrix
     */
    public static double[] randomRotation4D(long seed, double[] rotation3D) {
     ...
     */
    public static double[] randomRotation5D(long seed) {
     ...
     * @return a newly-allocated 25-element double array, meant as effectively a 5D rotation matrix
     */
    public static double[] randomRotation5D(long seed, double[] rotation4D) {
     ...
     */
    public static double[] randomRotation6D(long seed) {
     ...
     * @return a newly-allocated 36-element double array, meant as effectively a 6D rotation matrix
     */
    public static double[] randomRotation6D(long seed, double[] rotation5D) {
     ...
     */
    public static double[] randomRotation7D(long seed) {
     ...
     * used for noise.
     * @param seed any long; will be scrambled
     * @param rotation6D an existing 6D rotation matrix stored in a 1D double array; often produced by {@link #randomRotation6D(long)}
     * @return a newly-allocated 49-element double array, meant as effectively a 7D rotation matrix
     */
    public static double[] randomRotation7D(long seed, double[] rotation6D) {
     ...
     */
    public static double[] randomRotationArbitrary(long seed, int dimension) {
        dimension = Math.max(2, dimension);
        final int dimensionSq = dimension * dimension;
        final double[] base = fillRandomRotation2D(seed, new double[dimensionSq]);
        if(dimension > 2) {
     ...
     * @param matrix a square matrix as a 1D double array; should have length of at least {@code dimension * dimension}
     * @param dimension the side length of the square matrix
     * @return sb, for chaining
     */
    public static StringBuilder appendMatrix(StringBuilder sb, double[] matrix, int dimension) {
