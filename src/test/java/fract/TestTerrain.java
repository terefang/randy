...
import com.github.terefang.randy.fractal.impl.ExoTerrainFractal;
...
public class TestTerrain {
    ...
    public static int[] _OCT = { 0,1,2,3,4,8 };
    //public static float[] _FREQ = { 1.23f, 2.34f, 3.45f, 4.56f, 5.67f, 6.78f };
    public static float[] _FREQ = { 3.45f, 6.78f, 12.5f, 24.56f, 48.91f };
    //public static float[] _H = { .5f, .77f, 1.f, 1.23f, 1.5f };
    public static float[] _GAIN = { .5f };

    public static float[] _H = { .5f, .7f, .9f, 1.1f, 1.5f };
        ...
        for(final float _gain : _GAIN)
        {
            for(final float _freq : _FREQ)
            {
                for(int _oct : _OCT)
                {
                    for(float _h : _H)
                    {
                        for(final boolean _b : new boolean[]{ true, false})
                        {
                            final ITransform.TransformType _ttrans = ITransform.TransformType.T_NEAR;
                            //for(final ITransform.TransformType _ttrans : ITransform.TransformType.values())
                            {
                                for(final INoise _type : TestUtil.setupPerlinNoises(_seed)) {
                                    _EX.execute(() -> {
                                        final ExoTerrainFractal _ftype = new ExoTerrainFractal();
                                        _ftype.setBumps(_b);
                                        ...
                                        String _name = _ftype.name() + "/" + _type.name() + String.format("~fq=%04d,oct=%02d,G=%02d,H=%02d~%s", (int) (_freq * 10), _oct, (int) (_gain * 10), (int) (_h * 10), _ftype.isFractalSpiral() ? "S":"N");
                                        ...
                                        for (int _y = 0; _y < _SIZE; _y++) {
                                            for (int _x = 0; _x < _SIZE; _x++) {
                                                ...
                                                _nf.setPoint(_x, _y, _trans.transform(_value));
                                            }
                                        }
                                        ...
                                        _font.drawString(_g, 10,110, "H = " + _h, Color.YELLOW, Color.BLACK);
