            ...
            for(final float _freq : _FREQ)
            {
                for(int _oct : _OCT)
                {
                    ...
                    //for(final ITransform.TransformType _ttrans : ITransform.TransformType.values())
                    {
                        ...
                        {
                            final IFractal _ftype = RandyUtil.samplerFractal(_type, _oct, _freq, _gain,false);
                            _ftype.setLacunarity(_gain);
                            final ITransform _trans = TypeTransform.from(_ttrans);
                            _type.setTransform(_trans);
                            _EX.execute(() -> {
                                String _name = _ftype.name() + "/" + _type.name() + String.format("~fq=%04d,oct=%02d,G=%02d", (int) (_freq*10), _oct, (int)(_gain*10));
