...
public class TestOct
{
    ...
    static void execute(IFractal _ftype, INoise _type, ITransform.TransformType _ttrans, int _oct, float _freq, float _gain, long _seed)
    {
        ...
        _g.dispose();
        NoiseFieldUtil.savePNG(_bi, "./out/octfract/" + _name + ".png");
    ...
    public static void main(String[] args) {
        ...
        for(final float _gain : _GAIN)
        {
            for(final float _freq : _FREQ)
            {
                for(int _oct : _OCT)
                {
                    final ITransform.TransformType _ttrans = ITransform.TransformType.T_LEVEL8;
                    //for(final ITransform.TransformType _ttrans : ITransform.TransformType.values())
                    {
                        ...
                        {
                            _EX.execute(() -> {
                                AbstractFractal _F1 = RandyUtil.octFbmFractal(_type,_oct,_freq,_gain, true);
                                AbstractFractal _F2 = RandyUtil.bmFractal(_type,_freq,_gain,NoiseUtil.BASE_LACUNARITY);
                                
                                execute(_F1, _type,_ttrans,_oct,_freq,_gain,_seed);
                                execute(_F2, _type,_ttrans,_oct,_freq,_gain,_seed);
                            });
                            _EX.execute(() -> {
                                AbstractFractal _F1 = RandyUtil.octRidgedMultiFractal(_type,_oct,_freq,_gain, false);
                                AbstractFractal _F2 = RandyUtil.ridgedMultiFractal(_type,_freq,NoiseUtil.BASE_H,NoiseUtil.BASE_LACUNARITY);
                                
                                execute(_F1, _type,_ttrans,_oct,_freq,_gain,_seed);
                                execute(_F2, _type,_ttrans,_oct,_freq,_gain,_seed);
                            });
                            _EX.execute(() -> {
                                AbstractFractal _F1 = RandyUtil.octBillowFractal(_type,_oct,_freq,_gain, false);
                                AbstractFractal _F2 = RandyUtil.billowFractal(_type,_freq,NoiseUtil.BASE_H,NoiseUtil.BASE_LACUNARITY);
                                
                                execute(_F1, _type,_ttrans,_oct,_freq,_gain,_seed);
                                execute(_F2, _type,_ttrans,_oct,_freq,_gain,_seed);
                            });
                            _EX.execute(() -> {
                                AbstractFractal _F1 = RandyUtil.octTurbulenceFractal(_type,_oct,_freq,_gain, false);
                                AbstractFractal _F2 = RandyUtil.turbulenceFractal(_type,_gain,NoiseUtil.BASE_LACUNARITY);
